...
# ─── Setup Market (Forum) ─────────────────────────────────────────────
@bot.tree.command(name="setup_market",description="[DM] ติดตั้ง Market ลง Forum Channel (ทำครั้งเดียว)",guild=discord.Object(id=GUILD_ID))
@app_commands.describe(forum_channel="Forum Channel ที่ต้องการโพสต์ร้านค้า")
@is_admin()
async def setup_market(i:discord.Interaction, forum_channel:discord.ForumChannel):
    ...
    for shop_key, shop in SHOPS.items():
        try:
            # สร้าง embed หลัก
            embed = make_shop_embed(shop_key)
            embed.set_footer(text=f"💬 ร้านนี้เปิดตลอด 24/7 | ใช้ /buy <id> เพื่อซื้อ | อัปเดตล่าสุดโดย {i.user.display_name}")

            # หา tag ของ forum ถ้ามี
            tag = None
            for t in forum_channel.available_tags:
                if t.name.lower() in (shop_key, shop["name"].lower(), "shop", "store"):
                    tag = t
                    break

            # สร้าง thread ใหม่หรืออัปเดตอันเก่า
            existing = setup_data.get(shop_key)
            if existing:
                try:
                    thread = forum_channel.get_thread(existing["thread_id"])
                    if thread is None:
                        thread = await forum_channel.guild.fetch_channel(existing["thread_id"])
                    msg = await thread.fetch_message(existing["message_id"])
                    await msg.edit(embed=embed, view=ShopView(shop_key))
                    results.append(f"✅ อัปเดต **{shop['name']}** แล้ว")
                    continue
                except Exception:
                    pass  # thread หาย → สร้างใหม่

            # สร้าง thread ใหม่
            kwargs = {"name":f"{shop['emoji']} {shop['name']}", "content":"‎", "embed":embed, "view":ShopView(shop_key)}
            if tag: kwargs["applied_tags"] = [tag]
            thread, msg = await forum_channel.create_thread(**kwargs)
            ...
            results.append(f"✅ สร้าง **{shop['name']}** ใน {forum_channel.mention}")
        except Exception as e:
            results.append(f"❌ **{shop['name']}**: {e}")

    save_setup(setup_data)
    embed_result = discord.Embed(title="🏪 ติดตั้ง Market สำเร็จ!", color=0x2ECC71, description="\n".join(results))
    embed_result.set_footer(text="ร้านค้าจะคงอยู่ถาวร | ผู้เล่นสามารถเข้าไปดูได้ตลอดเวลา")
    await i.followup.send(embed=embed_result, ephemeral=True)
