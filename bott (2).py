...
SHOPS = {
    ...
    "general": {
        ...
        "npc":  "Mira Copperkettle",
        "desc": "ทุกอย่างที่นักผจญภัยต้องการ ตั้งแต่เชือก ไปจนถึงเสื้อคลุม "
                "ราคายุติธรรม ของครบ ไม่ว่าคุณจะออกเดินทางไปไหน",
        "emoji": "🎒",
        "color": 0x27AE60,
        "categories": {
...
def get_player(uid):
    ...
    if key not in players:
        return {"gold":0,"inventory":{},"quests":0,"level":3}
    return players[key]
...
# ─── Buy / Sell ───────────────────────────────────────────────────────
@bot.tree.command(name="gold", description="🪙 ดูทองของคุณ", guild=discord.Object(id=GUILD_ID))
async def gold_cmd(i: discord.Interaction):
    ...
    if inv:
        ...
        embed.add_field(name="📦 สิ่งของล่าสุด", value="\n".join(lines), inline=False)
...
class BuyRollSelect(discord.ui.Select):
    def __init__(self, item_ids: list, sold: set = None):
        self.item_ids = item_ids
        self.sold = sold or set()
        seen = set(); options = []
        for iid in item_ids:
            ...
            item = MAGIC_ITEMS.get(iid)
            if item:
                is_sold = iid in self.sold
                label = item["name"] + (" [SOLD]" if is_sold else "")
                options.append(discord.SelectOption(
                    label=label[:100], value=iid,
                    emoji=RARITY_EMOJI.get(item["rarity"],""),
                    description=f"{item['rarity']} | {fmt_price(item['price'])}",
                    default=False
                ))
        # กรองเฉพาะที่ยังไม่ถูกซื้อให้เลือกได้
        avail = [o for o in options if "[SOLD]" not in o.label]
        if avail:
            super().__init__(placeholder="เลือกซื้อไอเทม...", options=options,
                             min_values=1, max_values=len(avail))
        else:
            super().__init__(placeholder="✅ ไอเทมทั้งหมดถูกซื้อแล้ว", options=options,
    ...
    async def callback(self, i: discord.Interaction):
        ...
        if newly_sold and i.message:
            all_sold = self.sold | newly_sold
            new_view = RollView(self.item_ids, sold=all_sold)
            try:
...
class RollView(discord.ui.View):
    def __init__(self, item_ids: list, sold: set = None):
        ...
        self.add_item(BuyRollSelect(item_ids, sold=sold or set()))
...
@bot.tree.command(name="admin_quest", description="[DM] เพิ่มเควสให้ผู้เล่น (รองรับหลายคน)", guild=discord.Object(id=GUILD_ID))
@app_commands.describe(
    members="@mention ผู้เล่นได้หลายคน เช่น @A @B @C",
    amount="จำนวนเควสที่เพิ่ม (default 1)",
    quest_name="ชื่อเควส (ไม่บังคับ)"
)
@is_admin()
async def admin_quest(i: discord.Interaction, members: str, amount: int = 1, quest_name: str = None):
    if amount <= 0:
        await i.response.send_message("❌ จำนวนเควสต้องมากกว่า 0", ephemeral=True); return

    import re as _re
    member_ids = [int(m) for m in _re.findall(r"<@!?(\d+)>", members)]
    if not member_ids:
        await i.response.send_message("❌ ไม่พบ @mention ผู้เล่น — ลอง @mention ชื่อผู้เล่นในช่อง members", ephemeral=True); return

    results = []; levelup_mentions = []

    for uid in member_ids:
        member = i.guild.get_member(uid)
        if not member:
            try: member = await i.guild.fetch_member(uid)
            except: results.append(f"❌ ไม่พบ <@{uid}>"); continue

        old_data   = get_player(uid)
        old_quests = old_data.get("quests", 0)
        old_level  = calc_level(old_quests)

        def do_quest(p, _amt=amount):
            p.setdefault("quests", 0)
            p["quests"] += _amt
            p["level"]   = calc_level(p["quests"])

        player     = await update_player(uid, do_quest)
        ...
        if leveled_up:
            ...
            results.append(f"🎉 {member.mention} **Level Up!** {old_level} → **{new_level}** (+{levels_gained})")
            levelup_mentions.append(member.mention)
        else:
            to_next = quests_to_next(new_quests)
            results.append(f"✅ {member.mention} Lv.**{new_level}** | {new_quests} เควส | อีก {to_next} → Lv.{new_level+1}")

    has_levelup = bool(levelup_mentions)
    embed = discord.Embed(
        title="🎉 Level Up!" if has_levelup else "📜 บันทึกเควสสำเร็จ",
        color=0xFFD700 if has_levelup else 0x2ECC71
    )
    if quest_name:
        embed.add_field(name="⚔️ เควส", value=f'*"{quest_name}"*', inline=False)
    embed.add_field(name="📜 เควสที่ได้รับ", value=f"+{amount} เควส", inline=True)
    embed.add_field(name="👥 ผู้เล่น",       value=f"{len(member_ids)} คน", inline=True)
    embed.add_field(name="📋 ผลลัพธ์",       value="\n".join(results),      inline=False)
    if has_levelup:
        embed.set_footer(text="🎊 ยินดีด้วย " + ", ".join(levelup_mentions) + "!")
    await i.response.send_message(embed=embed)
